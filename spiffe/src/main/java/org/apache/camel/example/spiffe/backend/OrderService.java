...
public class OrderService {
    ...
    public Map<String, Object> listOrders(@Header(SpiffeConstants.SPIFFE_ID) String caller) {
    ...
    public Map<String, Object> withStock(
            @ExchangeProperty("orders") Map<String, Object> response, Map<String, Object> stockLevels) {
        for (Map<String, Object> order : (List<Map<String, Object>>) response.get("orders")) {
            int available = ((Number) stockLevels.getOrDefault(order.get("item"), 0)).intValue();
            order.put("inStock", available >= (Integer) order.get("quantity"));
        }
        return response;
    }

    private static Map<String, Object> order(int id, String item, int quantity) {
