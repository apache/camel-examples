...
#  2. registers the workloads, mapping the Unix user id of each application to its SPIFFE ID
...
register() {
    ...
    # the entry create command prints a verbose block per entry; the line above is enough, so hide its output
    "${SPIRE_BIN}/spire-server" entry create -socketPath "${SERVER_SOCKET}" \
        -parentID "${AGENT_ID}" \
        -spiffeID "spiffe://${TRUST_DOMAIN}/$1" \
        -selector "unix:uid:$2" > /dev/null
}
register assistant 2001
register public-chatbot 2002
register support-console 2003
