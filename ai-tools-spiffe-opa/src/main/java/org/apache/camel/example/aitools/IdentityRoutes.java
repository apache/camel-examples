 ...
 * <p>
 * Every application of this example runs this same route, yet each is issued a different identity: the SPIRE agent
 * attests the process that connects to the Workload API (here by its Unix user id) and looks up the registration entry
 * that matches it. Identity comes from the platform, not from the code or its configuration.
 * <p>
